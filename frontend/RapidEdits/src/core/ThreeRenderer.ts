...
export class ThreeRenderer {
  ...
  constructor(container: HTMLElement) {
    ...
    // 5. Placeholder (Gradient fallback)
    // We use a canvas texture to create a gradient matching the theme
    const canvas = document.createElement('canvas');
    canvas.width = 2;
    canvas.height = 2;
    const ctx = canvas.getContext('2d')!;
    const gradient = ctx.createLinearGradient(0, 0, 2, 2);
    gradient.addColorStop(0, '#1e3c72'); // Brand Primary
    gradient.addColorStop(1, '#2a5298'); // Brand Secondary
    ctx.fillStyle = gradient;
    ctx.fillRect(0, 0, 2, 2);
    
    const placeholderTexture = new THREE.CanvasTexture(canvas);
    placeholderTexture.magFilter = THREE.NearestFilter; // Sharp scaling
    
    this.placeholderMesh = new THREE.Mesh(
        this.planeGeometry,
        new THREE.MeshBasicMaterial({ map: placeholderTexture, opacity: 0.3, transparent: true })
    );
    // Put it way back
  ...
  private render() {
    ...
    visibleClips.forEach((clip) => {
        ...
        if (!mesh) {
          ...
          // Async Texture Load
          this.allocator.getTexture(editorEngine.getAsset(clip.assetId)!).then(texture => {
              ...
              if (texture && currentMesh === newMesh) {
                   console.log('Texture loaded for clip:', clip.id);
                   const mat = newMesh.material as THREE.MeshBasicMaterial;
                   mat.map = texture;
                   mat.needsUpdate = true;
                   this.fitMeshToScreen(newMesh, texture);
              } else {
                   console.warn('Texture failed or mesh stale for clip:', clip.id);
              }
          });
        }
  ...
  private syncVideoFrame(clip: Clip, mesh: THREE.Mesh, globalTime: number) {
     ...
     if (!material.map || !(material.map instanceof THREE.VideoTexture)) return;
