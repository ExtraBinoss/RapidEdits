...
export class PixiRenderer {
    ...
    private async loadSafeTexture(clip: Clip): Promise<Texture | null> {
        ...
        if (asset.type === "video") {
            ...
            // 2. Double Check Dimensions AND Force Delay
            // Even if readyState is 2, WebGL might need a tick to see the frame data
            if (video.videoWidth === 0 || video.videoHeight === 0) return null;

            // Mandatory short delay to allow browser video pipeline to stabilize frame 0
    ...
    private async syncVideo(clip: Clip, sprite: Sprite, globalTime: number) {
        ...
        const video = (sprite.texture.source as VideoSource)
            .resource as HTMLVideoElement;
        if (!video) return;

        // Time Sync
        let clipTime = globalTime - clip.start + clip.offset;

        // ROBUSTNESS: Force epsilon seek at 0.0 to prevent decoder initialization crashes
        // This addresses the "Level of detail outside of range" Error at t=0
        if (clipTime < 0.001) {
            clipTime = 0.001;
        }
        ...
        if (video.readyState >= 2) {
            ...
            if (source && !source.destroyed) {
                try {
                    ...
                    console.warn(`Texture update failed for ${clip.id}`, e);
                }
            }
        }
    }
    ...
    private handleResize() {
        ...
        this.clipSprites.forEach((s) => {
            if (s.texture && s.texture !== Texture.EMPTY)
                this.fitSpriteToScreen(s, s.texture);
        });
    }
